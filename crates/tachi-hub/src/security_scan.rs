...
pub fn resolve_security_scan_backend() -> SecurityScanBackend {
...
pub fn scan_skill_definition(def: &serde_json::Value) -> serde_json::Value {
...
pub fn normalize_review_status(status: &str) -> Option<&'static str> {
...
#[cfg(test)]
mod scan_tests {
    use super::{merge_skill_scans, normalize_review_status, scan_skill_definition};
    use serde_json::json;

    #[test]
    fn static_scan_blocks_high_risk_destructive_prompts() {
        let scan = scan_skill_definition(&json!({
            "prompt": "Run rm -rf /tmp/demo"
        }));

        assert_eq!(scan["risk"], "high");
        assert_eq!(scan["blocked"], true);
        assert!(scan["signals"]
            .as_array()
            .expect("signals")
            .iter()
            .any(|signal| signal == "destructive_action"));
    }

    #[test]
    fn merge_skill_scans_preserves_highest_risk_and_dedupes_findings() {
        let static_scan = json!({
            "risk": "medium",
            "blocked": false,
            "signals": ["secret_exposure"],
            "findings": ["Contains .env access"]
        });
        let llm_scan = json!({
            "status": "ok",
            "result": {
                "risk": "high",
                "blocked": true,
                "signals": ["secret_exposure", "data_exfiltration"],
                "findings": ["Contains .env access", "Can exfiltrate data"]
            }
        });
        ...
        assert_eq!(
            merged["findings"],
            json!(["Contains .env access", "Can exfiltrate data"])
        );
    }
