...
mod memory;
pub use memory::{
    clamp_facade_top_k, TachiMemoryAction, TachiMemoryParams, TachiSaveParams, TachiSearchParams,
...
/// #1099: `leave`/`check` were retired (see #1016 — the canonical Task/A2A
/// surfaces replace them) along with the briefing projection and GC branch that only
...
pub struct TachiHandoffParams {
    ...
    /// tachi_task(action='handoff') for work handoff and
    /// tachi_a2a(action='respond'|'status') for same-host advisory messages.
    pub action: String,
...
pub use action_inventory::{
    ...
    TACHI_MEMORY_ACTIONS, TACHI_MEMORY_ACTION_SOFT_MAX, TACHI_MEMORY_RETIRED_C2B_ACTIONS,
    TACHI_ORCHESTRATOR_ACTIONS, TACHI_SKILL_ACTIONS, TACHI_STAFF_ACTIONS,
    TACHI_TASK_PRIMARY_ACTION_SOFT_MAX, TACHI_TASK_REMOVED_GH_LIFECYCLE_ACTIONS,
    TACHI_TASK_RETIRED_ACTIONS, TACHI_TASK_RETIRED_C1A_ACTIONS, TACHI_TASK_RETIRED_C1C_ACTIONS,
    TACHI_TUNE_ACTIONS, TACHI_WIKI_ACTIONS,
};
