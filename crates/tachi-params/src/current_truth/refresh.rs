...
fn composite_issue_revision(issue: &SnapshotIssueV1, linked_prs: &[&SnapshotPrV1]) -> String {
    let prs: Vec<serde_json::Value> = linked_prs
        ...
        .map(|pr| {
            serde_json::json!({
                ...
                "state": pr.state,
                // Visibility participates in the composite identity: a
                // visibility-only change must mint a NEW revision, not
                // re-use an id whose content changed.
                "visibility": match pr.visibility {
    ...
    let basis = serde_json::json!({
        ...
        "issue_visibility": match issue.visibility {
            VisibilityClassV1::Public => "public",
            VisibilityClassV1::Private => "private",
        },
        "linked_prs": prs,
