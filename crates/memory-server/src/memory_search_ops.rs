...
pub(crate) async fn handle_save_memory(
    ...
    let status = if (needs_embedding || needs_summary)
        && should_enqueue_enrichment(&entry)
        && server.inline_enrichment_enabled()
    {
        "saved (inline enrichment attempted)"
    } else if (needs_embedding || needs_summary) && should_enqueue_enrichment(&entry) {
