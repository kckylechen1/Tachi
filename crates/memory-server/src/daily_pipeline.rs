...
pub(crate) async fn run_daily_pipeline(
    ...
    // ── Deep Sleep writeback: supersede source memories ──
    if let Err(e) = run_deep_sleep_writeback(server).await {
        eprintln!("[daily_pipeline] deep sleep writeback skipped: {e}");
    }

    // ── Deep Sleep wiki: distill key decisions into wiki entries ──
    if let Err(e) = run_deep_sleep_wiki_distill(server).await {
        eprintln!("[daily_pipeline] deep sleep wiki distill skipped: {e}");
    }
...
async fn run_truth_maintenance_stage(
    ...
    for target in targets.iter().filter(|target| target.allow_write) {
        let target_db = if target.role == "global" {
            DbScope::Global
        } else {
            DbScope::Project
        };
        run_truth_maintenance_for_target(server, target, target_db)?;
    }
    // ── consolidated → pattern promotion (async: requires rerank) ──
    for target in targets.into_iter().filter(|target| target.allow_write) {
        if let Err(e) = run_pattern_promotion_for_target(server, &target).await {
            eprintln!("[daily_pipeline] pattern promotion for {}: {e}", target.label);
        }
    }
    Ok(())
}

fn run_truth_maintenance_for_target(
    server: &MemoryServer,
...
/// Consolidated → pattern promotion (Deep Sleep gate).
///
/// Queries consolidated memories meeting OpenClaw spec criteria, reranks them
/// via Voyage for pattern-worthiness, and promotes entries with score ≥ 0.8.
/// This is a separate async function from `run_truth_maintenance_for_target`
/// because it requires an `.await` for rerank and the synchronous truth
/// maintenance holds a `Connection` (which is !Send).
async fn run_pattern_promotion_for_target(
    server: &MemoryServer,
    target: &ManifestDbTarget,
) -> Result<(), String> {
    let Some(db_path) = target.path.to_str() else {
    ...
    let pattern_candidates: Vec<String> = {
        let store = MemoryStore::open_with_label(db_path, &target.label)
            .map_err(|e| format!("open for pattern scan {}: {e}", target.label))?;
        let conn = store.connection();
        let mut stmt = conn
            .prepare(
                "SELECT id FROM memories
                 WHERE archived = 0
                   ...
                   AND query_diversity >= 3
                   AND importance >= 0.8
                   AND julianday('now') - julianday(COALESCE(NULLIF(created_at, ''), timestamp)) <= 30
                 ORDER BY importance DESC, recall_count DESC
                 LIMIT 10",
            )
            .map_err(|e| format!("prepare pattern promotion query {}: {e}", target.label))?;
        let ids: Vec<String> = stmt
            .query_map([], |r| r.get(0))
            .map_err(|e| format!("query pattern candidates {}: {e}", target.label))?
            .filter_map(Result::ok)
            .collect();
        drop(stmt);
        drop(store);
    ...
    // Phase 2: fetch entry text for rerank — aligned with candidate IDs
    let (rerank_ids, docs): (Vec<String>, Vec<String>) = {
        let store = MemoryStore::open_with_label(db_path, &target.label)
            .map_err(|e| format!("reopen for pattern fetch {}: {e}", target.label))?;
        let conn = store.connection();
        let candidate_entries =
            memory_core::db::fetch_by_ids(conn, &pattern_candidates, false)
                .unwrap_or_default();
        // Build (id, doc) pairs in candidate order; skip IDs not returned by fetch.
        let pairs: Vec<(String, String)> = pattern_candidates
            .iter()
            .filter_map(|id| {
                candidate_entries.get(id).map(|e| {
                    let doc = format!(
                        "{}\n{}",
                        e.summary,
                        e.text.chars().take(500).collect::<String>()
                    );
                    (id.clone(), doc)
                })
            })
    ...
    // Phase 3: async rerank
    let rerank_query = "What durable architectural decisions, design patterns, or engineering knowledge would be most valuable as a long-term reference?";
    match server.llm.rerank_voyage(rerank_query, &docs, 10).await {
        Ok(scored) => {
            // Phase 4: promote high-scoring entries
            let store = MemoryStore::open_with_label(db_path, &target.label)
                .map_err(|e| format!("reopen for pattern promote {}: {e}", target.label))?;
            let conn = store.connection();
            for (idx, score) in scored {
                if score >= 0.8 {
                    if let Some(id) = rerank_ids.get(idx) {
                        if let Err(e) = conn.execute(
                            "UPDATE memories SET tier = 'pattern', updated_at = datetime('now') WHERE id = ?1",
                            rusqlite::params![id],
                        ) {
                            eprintln!("[deep-sleep] failed to promote {id} to pattern: {e}");
                        }
                    }
                }
            }
        ...
        Err(e) => {
            eprintln!("[deep-sleep] rerank for pattern promotion failed: {e}");
        }
    }
    Ok(())
}

/// Deep Sleep writeback: supersede source memories with their distilled versions.
///
/// For each consolidated memory produced by `foundry_distill` in the last 24h,
/// reads its `metadata.source_memory_ids` and calls `supersede_memory` so the
/// old raw fragments become invisible in search while remaining for audit.
pub(crate) async fn run_deep_sleep_writeback(
    server: &MemoryServer,
) -> Result<(), String> {
    let do_writeback = |store: &mut MemoryStore| -> Result<usize, String> {
        let conn = store.connection();
        let mut stmt = conn
            .prepare(
                "SELECT id, metadata FROM memories
                 WHERE archived = 0
                   ...
                   AND json_extract(metadata, '$.source_memory_ids') IS NOT NULL",
            )
            .map_err(|e| format!("prepare writeback query: {e}"))?;

        let rows: Vec<(String, String)> = stmt
            .query_map([], |row| {
                Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
            })
            .map_err(|e| format!("query writeback candidates: {e}"))?
            .filter_map(Result::ok)
            .collect();

        let mut superseded = 0usize;
        for (distill_id, metadata_json) in &rows {
            let meta: Value =
            ...
            if let Some(ids) = source_ids {
                for sid in ids.iter().filter_map(Value::as_str) {
                    match memory_core::db::supersede_memory(conn, sid, distill_id) {
                        Ok(true) => superseded += 1,
                        Ok(false) => {} // already superseded or same id
                        Err(e) => eprintln!(
                            "[deep-sleep-writeback] supersede {sid} → {distill_id}: {e}"
                        ),
                    }
                }
            }
    ...
    if let Ok(n) = server.with_global_store(do_writeback) {
        total += n;
    }
    if server.has_project_db() {
        if let Ok(n) = server.with_project_store(|store: &mut MemoryStore| do_writeback(store)) {
            total += n;
        }
    }
    if total > 0 {
        eprintln!("[deep-sleep-writeback] superseded {total} source memories");
    }
    Ok(())
}

/// Deep Sleep wiki distill: generate wiki entries from recently promoted pattern memories.
///
/// Queries pattern-tier memories created in the last 24h, groups by domain, and
/// uses the Distill LLM (cheap Qwen 27B) to produce one wiki entry per domain group.
/// Entries are written directly to `/wiki/<domain>/<slug>` and marked with
/// `metadata.wiki_distilled=1` to avoid re-processing.
pub(crate) async fn run_deep_sleep_wiki_distill(
    server: &MemoryServer,
) -> Result<(), String> {
    use rmcp::handler::server::wrapper::Parameters;

    /// Helper type for wiki distill candidate rows: (id, summary, text, domain, scope)
    struct WikiDistillRow {
        id: String,
        summary: String,
        text: String,
        domain: String,
    ...
    // Collect recently promoted pattern memories grouped by domain.
    let collect = |store: &mut MemoryStore| -> Result<Vec<WikiDistillRow>, String> {
        let conn = store.connection();
        let mut stmt = conn
            .prepare(
                "SELECT id, summary, text, domain, COALESCE(scope, 'global')
                 FROM memories
                 WHERE archived = 0
                   AND tier = 'pattern'
                   AND created_at > datetime('now', '-1 day')
                   AND (json_extract(metadata, '$.wiki_distilled') IS NULL
                        OR json_extract(metadata, '$.wiki_distilled') = 0)
                 ORDER BY importance DESC
                 LIMIT 50",
            )
            .map_err(|e| format!("prepare wiki distill query: {e}"))?;
        let rows = stmt
            .query_map([], |row| {
                Ok(WikiDistillRow {
            ...
            .map_err(|e| format!("query wiki distill candidates: {e}"))?;
        Ok(rows.filter_map(Result::ok).collect())
    };

    let mut rows = Vec::new();
    if let Ok(global) = server.with_global_store(collect) {
        rows.extend(global);
    }
    if server.has_project_db() {
        if let Ok(project) = server.with_project_store(|store: &mut MemoryStore| collect(store)) {
            rows.extend(project);
        }
    }
    if rows.is_empty() {
    ...
    // Group by domain
    let mut by_domain: std::collections::HashMap<String, Vec<WikiDistillRow>> =
    ...
    for row in rows {
        ...
        by_domain.entry(domain).or_default().push(row);
    }

    let wiki_prompt = r#"You are generating a concise wiki entry from a set of related engineering pattern memories.
Return ONLY a single valid JSON object:
{
  "title": "<clear noun-phrase title, ≤80 chars>",
  "body": "<structured Markdown, 200–800 chars>",
  "summary": "<one sentence TL;DR, ≤120 chars>"
}
Rules:
1. Write as forward-looking reference: what to do, why, and known gotchas.
2. Remove session noise: timestamps, tool traces, raw JSON.
3. Keep concrete details: crate names, error signatures, config keys.
...
"#;
    ...
    for (domain, members) in by_domain {
        let combined: String = members
            .iter()
            .map(|m| format!("## {}\n{}", m.summary, m.text.chars().take(600).collect::<String>()))
            .collect::<Vec<_>>()
            .join("\n\n");

        let raw = server
            .llm
            .call_distill_llm(wiki_prompt, &combined, None, 0.2, 800)
            .await
            .map_err(|e| format!("wiki distill LLM for domain '{domain}': {e}"))?;

        // Parse the JSON response
        let parsed = crate::llm::LlmClient::extract_json_payload(&raw)
            .ok()
        ...
        let body = parsed
            ...
            .filter(|s| !s.trim().is_empty())
            .map(|s| s.to_string())
            .unwrap_or_else(|| combined.chars().take(500).collect());
        let summary = parsed
            .get("summary")
            .and_then(Value::as_str)
            .unwrap_or("")
        ...
        let slug = title
            ...
            .collect::<Vec<_>>()
            .join("-");
        let path = format!("/wiki/{domain}/{slug}");

        // Determine scope: use the first member's scope
        let scope = members
            .first()
            .map(|m| m.scope.clone())
        ...
        let _ = server
            .tachi_save(Parameters(crate::tool_params::TachiSaveParams {
                text: body,
                id: None,
                ...
                keywords: vec!["deep-sleep-wiki".to_string(), domain.clone()],
                entities: Vec::new(),
                scope: Some(scope),
                project: None,
                ...
                force: true,
                topic: Some(domain.clone()),
                source: Some("deep_sleep_wiki_distill".to_string()),
                valid_from: None,
                valid_until: None,
            }))
            .await
            .map_err(|e| format!("wiki distill save for '{domain}': {e}"))?;

        // Mark source entries as wiki_distilled
        let member_ids: Vec<String> = members.iter().map(|m| m.id.clone()).collect();
        let mark = |store: &mut MemoryStore| -> Result<(), String> {
            let conn = store.connection();
            for id in &member_ids {
                let _ = conn.execute(
                    r#"UPDATE memories SET metadata = json_set(
        ...
        let _ = server.with_global_store(mark);
        if server.has_project_db() {
            let _ = server.with_project_store(|store: &mut MemoryStore| mark(store));
        }

        eprintln!("[deep-sleep-wiki] wrote wiki entry for domain '{domain}' at {path}");
    }
    Ok(())
}
