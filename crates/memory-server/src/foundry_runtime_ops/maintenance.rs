...
async fn handle_foundry_maintenance_item(
    ...
    let lease = with_foundry_store(server, item, |store| {
        memory_core::claim_foundry_job_for_run(store.connection(), &item.job.id, &running_cutoff)
    ...
    let result = match item.job.kind {
        ...
        memory_core::FoundryJobKind::ForgetSweep => {
            process_forget_sweep_job(server, item).map(|_| {
                FoundryMaintenanceOutcome::Terminal(memory_core::FoundryJobStatus::Completed, None)
            })
        }
...
pub(crate) async fn run_foundry_maintenance_worker(
    ...
    while let Some(item) = rx.recv().await {
        ...
        match result {
            ...
            Ok(FoundryMaintenanceOutcome::Terminal(memory_core::FoundryJobStatus::Skipped, _)) => {
                server
                    .foundry_lock()
                    .foundry_stats
                    .skipped
