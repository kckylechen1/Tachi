...
fn auto_daemon_command_args(
    ...
    let mut args = vec![
        ...
        std::ffi::OsString::from(port),
        std::ffi::OsString::from("--global-db"),
        global_db_path.as_os_str().to_owned(),
    ];
    match project_db_path {
        Some(project_db_path) => {
            args.push(std::ffi::OsString::from("--project-db"));
            args.push(project_db_path.as_os_str().to_owned());
        }
        None => args.push(std::ffi::OsString::from("--no-project-db")),
    }
    args
}
    ...
    if embedded_mcp_facade() {
        ...
        if daily_distill_scheduler_enabled(&server) {
            ...
            tokio::spawn(async move {
                ...
                let mut interval =
                    tokio::time::interval(Duration::from_secs(distill_interval_secs));
...
mod tests {
    ...
    fn auto_daemon_args_preserve_global_only_scope() {
        let args = auto_daemon_command_args(std::path::Path::new("/tmp/agent.db"), None, "0");
    ...
    fn auto_daemon_args_preserve_project_scope() {
        let args = auto_daemon_command_args(
            std::path::Path::new("/tmp/global.db"),
            Some(std::path::Path::new("/tmp/project.db")),
            "1234",
        );
        let rendered: Vec<String> = args
            .iter()
