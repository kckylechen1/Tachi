...
#[tokio::test]
async fn dispatch_vault_env_injection_fills_missing_keys_without_overriding_env() {
    ...
    for (name, value) in [
        ...
        ("TACHI_EXISTING_API_KEY", "vault-should-not-override"),
    ] {
        server
            .vault_set(Parameters(VaultSetParams {
                name: name.to_string(),
    ...
    cmd.arg("-c")
        .arg("printf '%s|%s' \"$TACHI_CHILD_ONLY_API_KEY\" \"$TACHI_EXISTING_API_KEY\"");
    let injected = crate::dispatch_ops::apply_unlocked_vault_env(&mut cmd, &server);
    assert!(injected >= 1);
    ...
    assert_eq!(stdout, "child-vault-value|env-value");
