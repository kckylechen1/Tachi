...
async fn g4_complete_default_omits_subagent_notes_sentinel() {
    ...
    assert!(bundle.get("eval_entry").is_some());
    assert!(
        resp.len() < 1200,
        "complete receipt too large: {} bytes",
        resp.len()
    );
}

#[tokio::test]
async fn g4_complete_receipt_with_pr_ref_omits_echo() {
    let server = make_server();

    let resp = server
        .tachi_complete(Parameters(TachiCompleteParams {
            task_id: Some("g528-complete-pr".to_string()),
            task: format!("Task with {ECHO_SENTINEL}"),
            agent: "claude-code".to_string(),
            outcome: "success".to_string(),
            task_type: Some("fix_request".to_string()),
            ...
            flow_id: None,
            issue_ref: None,
            pr_ref: Some("owner/repo#42".to_string()),
            evidence_refs: Vec::new(),
            tests_run: Vec::new(),
            diff_present: None,
            scope: Some("global".to_string()),
    ...
    assert!(
        !resp.contains(ECHO_SENTINEL),
        "complete receipt echoed task text: {resp}"
    );
    let bundle: Value = serde_json::from_str(&resp).expect("bundle");
    assert_eq!(bundle["pr_ref"], json!("owner/repo#42"));
    assert!(bundle.get("eval_entry").is_some());
    assert!(bundle.get("pipeline").is_some());
    assert!(bundle.get("next_steps").is_some());
}
