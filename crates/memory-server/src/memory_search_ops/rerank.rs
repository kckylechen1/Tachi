...
//! order with a hybrid-score floor (pure math in `memory_core::search`).
//!
//! History:
//! - Lived in `foundry_runtime_ops::recall`; relocated here to cut the
//!   `memory_search_ops -> foundry_runtime_ops` dependency edge (#876).
//! - Pure RRF + hybrid-floor blend then moved into `memory-core` so
...
use memory_core::merge_rerank_order_with_hybrid_floor;
