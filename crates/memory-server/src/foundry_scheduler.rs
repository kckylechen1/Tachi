...
// Manual Debug for Route since Route::Orphan(&'static str) auto-derives
// fine, but tests assert via panic format — keep an explicit Debug to
// guarantee output stability across rust versions.
impl std::fmt::Debug for Route {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Route::Global => write!(f, "Global"),
            Route::Project => write!(f, "Project"),
            Route::NamedProject(n) => write!(f, "NamedProject({n})"),
            Route::Orphan(r) => write!(f, "Orphan({r})"),
        }
    }
}
