...
#[derive(Clone)]
pub(super) struct ChatLaneConfig {
    pub(super) base_url: String,
    pub(super) model: String,
    pub(super) api_key_envs: Vec<&'static str>,
}
...
#[derive(Clone)]
pub(super) struct SelectedProviderSecret {
...
pub(super) enum KeyAvailability {
    Available,
    Cooldown,
    AuthFailed,
    Disabled,
...
pub(super) struct ProviderHealthSnapshot {
    pub(super) availability: KeyAvailability,
    pub(super) cooldown_until: Option<DateTime<Utc>>,
...
impl ProviderHealthSnapshot {
    fn from_health(health: &VaultKeyHealth) -> Self {
        Self::from_health_parts(
            health,
            super::LlmClient::parse_timestamp(&health.updated_at),
            health
                .cooldown_until
                .as_deref()
                .and_then(super::LlmClient::parse_timestamp),
        )
    }
    ...
    fn from_health_parts(
        ...
        match super::LlmClient::status_from_key_health_status(health.status.as_str()) {
            HEALTH_RATE_LIMITED => {
...
pub(super) struct ProviderState {
    pub(super) secrets: HashMap<String, Vec<ProviderSecret>>,
    pub(super) cooldowns: HashMap<String, Instant>,
    pub(super) indices: HashMap<String, usize>,
    pub(super) health: HashMap<String, HashMap<String, VaultKeyHealth>>,
    pub(super) health_snapshots: HashMap<String, HashMap<String, ProviderHealthSnapshot>>,
}

#[derive(Debug, Clone)]
pub(super) struct ProviderHealthReloadState {
...
pub(super) struct ProviderHealthPersistState {
    last_attempt_at: Option<String>,
    last_success: Option<Instant>,
    last_success_at: Option<String>,
    last_error: Option<String>,
...
#[derive(Debug, Clone)]
pub(super) struct ClaudeCliFailure {
    pub(super) kind: ClaudeCliFailureKind,
    pub(super) failed_at: Instant,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum ClaudeCliFailureKind {
    SpawnFailed,
    Timeout,
}

impl ClaudeCliFailureKind {
    pub(super) fn from_error(error: &str) -> Option<Self> {
...
pub(super) struct ClaudeCliSkip {
    pub(super) kind: ClaudeCliFailureKind,
