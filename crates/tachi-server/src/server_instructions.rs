...
(1b) issue/PR lifecycle navigation when a flow, issue, or PR exists: tachi_task(action='cycle_plan', flow_id=..., issue_ref=..., or pr_ref=...) before advisory recommend, PR handoff, or close-loop; call tachi_staff(action='start', task=..., staffing_reason=...) only for an explicit native-first exception. \
