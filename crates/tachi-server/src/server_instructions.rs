...
pub(crate) fn mcp_server_instructions() -> String {
    "Tachi — memory + policy/ledger copilot for coding agents, not the default worker launcher. Use the host harness's native subagent for ordinary local delegation. A Tachi-owned worker launch is only for an explicit user request, durable cross-session work, cross-device/remote pickup, or when no native subagent is available (use tachi_arena(action='spawn', launch=true, dispatch_reason=...)). \
WORKFLOW: (1) briefing at session start: tachi_memory(action='briefing'). \
(1b) issue/PR lifecycle navigation when a flow, issue, or PR exists: tachi_task(action='cycle_plan', flow_id=..., issue_ref=..., or pr_ref=...) before advisory recommend, PR handoff, or close-loop. \
