...
pub(super) fn feature_next_action(
    ...
    if memory_rows.is_empty() {
        return "Start with tachi_task(action='briefing') or save a checkpoint after the next concrete decision.".to_string();
    }
    "Inspect tachi_task(action='profiles') for card evidence, then use a harness-native subagent or review with explicit verification.".to_string()
}
