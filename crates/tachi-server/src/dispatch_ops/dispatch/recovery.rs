...
/// no self-report to preserve). `project` is passed `None`: recovery reads
/// only the on-disk `status.json`, which never carries the dispatch's
/// `TachiDispatchParams::project`, so this always takes the default
/// `resolve_write_scope("")` branch — a documented, narrower gap, not a
/// regression from the other three call sites.
