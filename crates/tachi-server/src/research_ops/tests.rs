...
//! `TACHI_RESEARCH_ALLOW_LOCAL_FETCH` escape hatch.
...
async fn feed_mode_treats_injection_text_as_quoted_data() {
    ...
    // The injection text lands ONLY inside the fenced "UNTRUSTED" excerpt block,
    // as quoted data — never promoted into an action.
    let report_path = response["report_path"].as_str().unwrap();
    let report = std::fs::read_to_string(report_path).expect("read report.md");
    assert!(
    ...
    assert!(
        ...
        "injection text is preserved verbatim as quoted evidence"
    );
