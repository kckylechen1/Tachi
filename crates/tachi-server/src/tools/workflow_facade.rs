...
impl MemoryServer {
    ...
    #[tool(
        description = "Task memory, policy, and ledger facade. The host harness's native subagent is the DEFAULT for ordinary local delegation; worker launch is tachi_staff(action='start'), not tachi_task. action='brief': feature-scoped handoff board with layered docs/specs, run artifacts, board state, wiki, memory fragments, eval evidence, and next action; action='profiles'/'profile'/'card': inspect cards; action='status'/'board': read existing worker state, with status also returning a nested read-only cycle view when flow_id/issue_ref/pr_ref is supplied (Task is a unified work read model, not a worker-status authority); action='complete': record evaluated completion evidence; action='intake': bind lifecycle inputs; action='build_references'/'close_loop': closure records. GitHub PR lifecycle is tachi_gh only. Route-policy tuning is tachi_tune only (admin/operator). Harness-native subagent outcomes can be mirrored into the existing eval/ledger surfaces without Tachi owning their process lifecycle."
    )]
