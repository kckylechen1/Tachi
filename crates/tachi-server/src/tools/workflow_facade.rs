...
impl MemoryServer {
    ...
    // ─── Facade: task (plan / recommend / complete / status / board / lifecycle)

    #[tool(
        description = "Task memory, policy, and ledger facade. The host harness's native subagent is the DEFAULT for ordinary local delegation; worker launch is tachi_staff(action='start'), not tachi_task. action='briefing': feature-scoped handoff board with docs/specs, run artifacts, board state, wiki, memory fragments, eval evidence, and next action; action='doc_index': project-first layered source index; action='plan': search memory/wiki and produce a todo list; action='recommend': advisory profile/card evidence, not permission or an instruction to launch a worker; action='route_simulate'/'proposals'/'review_proposal'/'apply_proposals': routing-policy evaluation; action='profiles'/'profile'/'card': inspect cards; action='status'/'board': read existing worker state (Task is a unified work read model, not a worker-status authority); action='complete': record evaluated completion evidence; action='intake'/'cycle_status'/'cycle_plan': lifecycle read models; action='ux_matrix'/'build_references'/'close_loop': closure records; action='merge': local Tachi-dispatched worktree merge only. GitHub PR lifecycle is tachi_gh only. Harness-native subagent outcomes can be mirrored into the existing eval/ledger surfaces without Tachi owning their process lifecycle."
    )]
