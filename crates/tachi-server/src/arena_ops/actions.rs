...
use super::render::render_prompt_md;
use super::state::{
    active_state, arena_dir, arena_root, compact_mission_status, dispatch_response_summary,
    mission_dir, mission_file_nonempty, mission_statuses, new_mission_id, read_json_file,
    read_linked_dispatch_result, read_mission_plan, read_mission_result,
...
pub(crate) async fn handle_tachi_arena(
    ...
    match params.action.to_ascii_lowercase().as_str() {
        ...
        other => Err(format!(
            "Invalid action '{other}'. Use spawn, board, or collect."
        )),
    }
}
