...
use memcore::{AuthorityLevel, EffectScope, ProjectionKind, TachiEventQuery, TachiEventRecord};
use serde_json::json;
...
fn emit_event(server: &MemoryServer, params: TachiEventParams) -> Result<String, String> {
    ...
    let event = TachiEventRecord {
        ...
        adapter: trim_string(params.adapter.clone()).unwrap_or_else(|| "tachi-server".to_string()),
