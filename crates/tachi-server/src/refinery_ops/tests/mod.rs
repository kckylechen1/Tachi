...
//! | `live_relation_signals.rs` | #1105 (live per-relation evidence + commit-reachability shipped checks) | full pipeline via `build_refinery_packet_with_live_signals`, with a hand-built `LiveRelationSignals` standing in for `collect_live_relation_signals`'s async orchestration. The `gh`/`git` SHELLING inside that async orchestrator (and inside `GitRefResolver`) is not unit-tested here — same "live shelling untested, pure assembly tested" split as `doc_resolver`/`GitRefResolver` elsewhere in this tree. Fix-round-2 (PR #1191 checkpoint 1, "collector-level failure-discrimination tests"): the COLLECTOR-LEVEL DECISION LOGIC that consumes a fetch/reachability failure — whether "no evidence found" is a confirmed negative or must degrade to v1's `Unknown` — is now pure and IS unit-tested, in `refinery_ops::live_signals`'s own `#[cfg(test)]` module (`derive_live_relation_signals_incomplete_scan_stays_unknown_not_unshipped`, `derive_live_relation_signals_unavailable_reachability_stays_unknown_not_unshipped`, and their complete-scan/confirmed-negative counterparts), alongside `classify_related_state`/`pick_shipped_evidence`'s existing pure-assembly tests. |
//!
