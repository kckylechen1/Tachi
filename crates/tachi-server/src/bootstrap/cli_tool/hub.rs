...
pub(super) async fn run_hub_command(
    ...
    let hub_db = tachi_hub_cli::resolve_hub_db(None, app_home);
    match action {
        HubAction::List {
            cap_type,
            all,
        ...
        HubAction::Show { id } => {
            tachi_hub_cli::run(&HubAction::Show { id }, &hub_db, app_home)
                .map_err(std::io::Error::other)?;
            Ok(())
        }
        HubAction::Packs { all } => {
            tachi_hub_cli::run(&HubAction::Packs { all }, &hub_db, app_home)
                .map_err(std::io::Error::other)?;
            Ok(())
        }
        ...
        HubAction::Bindings => {
            tachi_hub_cli::run(&HubAction::Bindings, &hub_db, app_home)
                .map_err(std::io::Error::other)?;
            Ok(())
        }
        HubAction::Doctor { fix } => {
            tachi_hub_cli::run(&HubAction::Doctor { fix }, &hub_db, app_home)
                .map_err(std::io::Error::other)?;
            Ok(())
        }
        HubAction::Register {
        ...
        HubAction::Stats { json: false } => {
            tachi_hub_cli::cmd_stats(&hub_db).map_err(|e| std::io::Error::other(e.to_string()))?;
            Ok(())
        }
    }
}
