...
pub(super) async fn run_cli_command(
    ...
    match command {
        ...
        Commands::Build { .. } => {
            unreachable!("Build is handled in async context before generic CLI dispatch")
        }
        Commands::Harness { .. } => {
        ...
        Commands::InjectionSurface { .. } => {
            unreachable!("InjectionSurface is handled in async context before generic CLI dispatch")
        }
        Commands::Poke { .. } => {
