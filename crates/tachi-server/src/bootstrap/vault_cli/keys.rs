...
pub(super) fn derive_verified_vault_key_from_password(
    ...
    // (fail-closed on a malformed/unsupported value) before verifying the
...
pub(super) fn decrypt_named_secret_value(
    ...
    {
        return Err(format!(
            "Access denied: direct CLI cannot read agent-restricted secret '{name}'"
        )
        .into());
    }
