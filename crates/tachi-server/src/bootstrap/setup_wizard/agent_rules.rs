...
- `tachi_task` no longer owns worker launch (`action=\"dispatch\"`/`wait`/`cancel` were removed): use the host harness's native subagent, or `tachi_arena(action=\"spawn\", launch=true, dispatch_reason=...)` for an explicit durable/remote exception. Tachi availability, parallelism, tracking, or vendor choice alone is not a reason.\n\n\
