...
fn approve_merge_rejects_protected_branch() {
    let err = tachi_merge_ops::validate_static_merge_safety(
        "main",
...
fn approve_merge_rejects_branch_that_looks_like_git_option() {
    let err = tachi_merge_ops::validate_merge_branch_name("--ff-only")
...
fn approve_merge_rejects_repo_root_as_worktree() {
    ...
    assert!(tachi_merge_ops::worktree_equals_repo_root(
        "/tmp/repo-root",
        "/tmp/repo-root"
    ));
}
