...
#[tokio::test]
async fn tachi_tune_recall_proposals_review_and_apply_config_env() {
    ...
    let body = handle_tachi_tune_for_test(&server, proposals)
        .await
    ...
    let mut review = tachi_tune_params("recall_review");
    ...
    let review_body = handle_tachi_tune_for_test(&server, review)
        .await
    ...
    let mut missing_confirm = tachi_tune_params("recall_apply");
    missing_confirm.proposal_id = Some(proposal_id.clone());
    let err = handle_tachi_tune_for_test(&server, missing_confirm)
        .await
        .expect_err("apply should require confirm=true");
    assert!(err.contains("confirm=true"));

    let mut apply = tachi_tune_params("recall_apply");
    apply.format = Some("json".to_string());
    apply.proposal_id = Some(proposal_id);
    apply.confirm = true;
    let apply_body = handle_tachi_tune_for_test(&server, apply)
        .await
...
async fn recall_proposal_reject_stamps_a_ttl_immediately() {
    ...
    // (the same eval-input path a repeat `tachi_tune(action='recall_proposals',
    ...
    let mut regenerate = tachi_tune_params("recall_proposals");
    ...
    let regen_body = handle_tachi_tune_for_test(&server, regenerate)
        .await
...
async fn recall_regen_with_changed_evidence_does_not_inherit_approval() {
    ...
    let mut proposals_a = tachi_tune_params("recall_proposals");
    ...
    let body_a = handle_tachi_tune_for_test(&server, proposals_a)
        .await
    ...
    let mut proposals_b = tachi_tune_params("recall_proposals");
    ...
    let body_b = handle_tachi_tune_for_test(&server, proposals_b)
        .await
...
",
    ...
    review.proposal_id = Some(proposal_id.clone());
    review.review_status = Some("approved".to_string());
    let _ = handle_tachi_tune_for_test(&server, review)
        .await
        .expect("approve");
...
",
    ...
    apply.confirm = true;
    let body = handle_tachi_tune_for_test(&server, apply)
        .await
...
async fn recall_apply_completed_recovery_fsyncs_file_before_terminal_state() {
    ...
    review.proposal_id = Some(proposal_id.clone());
    review.review_status = Some("approved".to_string());
    handle_tachi_tune_for_test(&server, review)
        .await
    ...
    let mut first_apply = tachi_tune_params("recall_apply");
    ...
    let err = handle_tachi_tune_for_test(&server, first_apply)
        .await
    ...
    let mut blocked_retry = tachi_tune_params("recall_apply");
    ...
    let err = handle_tachi_tune_for_test(&server, blocked_retry)
        .await
    ...
    let mut retry = tachi_tune_params("recall_apply");
    retry.format = Some("json".to_string());
    retry.proposal_id = Some(proposal_id);
    retry.confirm = true;
    let body = handle_tachi_tune_for_test(&server, retry)
        .await
...
async fn recall_apply_same_content_inode_replacement_after_sync_refuses_terminal_cas() {
    ...
    let err = handle_tachi_tune_for_test(&server, apply)
        .await
...
async fn recall_terminal_state_cannot_be_rereviewed() {
    ...
    let mut reject = tachi_tune_params("recall_review");
    reject.proposal_id = Some(proposal_id.clone());
    reject.review_status = Some("rejected".to_string());
    let _ = handle_tachi_tune_for_test(&server, reject)
        .await
        .expect("reject");
    ...
    let mut re_approve = tachi_tune_params("recall_review");
    re_approve.proposal_id = Some(proposal_id.clone());
    re_approve.review_status = Some("approved".to_string());
    let err = handle_tachi_tune_for_test(&server, re_approve)
        .await
...
async fn recall_two_applies_yield_one_terminal_receipt() {
    ...
    apply.proposal_id = Some(proposal_id.clone());
    apply.confirm = true;
    let first_body = handle_tachi_tune_for_test(&server, apply.clone())
        .await
        .expect("apply #1");
...
async fn recall_review_refuses_tampered_unbound_top_level_config_env() {
    ...
    review.proposal_id = Some(proposal_id.clone());
    review.review_status = Some("approved".to_string());
    let err = handle_tachi_tune_for_test(&server, review)
        .await
        .expect_err(
...
async fn recall_apply_preserves_private_config_mode() {
    ...
    apply.proposal_id = Some(proposal_id);
    apply.confirm = true;
    handle_tachi_tune_for_test(&server, apply)
        .await
