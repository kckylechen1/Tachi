...
#[tokio::test]
#[allow(clippy::await_holding_lock)] // make_server uses process-wide fixture state
async fn provider_pool_access_batch_rolls_back_all_touches_on_touch_failure() {
    let _guard = crate::utils::global_test_lock()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let server = make_server();
    server
        .vault_init(Parameters(VaultInitParams {
            password: "access-count-rollback-password".to_string(),
        }))
        .await
        .expect("vault_init should succeed");
    server
        .vault_set_api_key_pool(Parameters(VaultSetApiKeyPoolParams {
            prefix: "ACCESS_COUNT_ROLLBACK_API_KEY".to_string(),
            values: vec![
                "fixture-pool-key-1".to_string(),
                "fixture-pool-key-2".to_string(),
                "fixture-pool-key-3".to_string(),
            ...
            agent_id: None,
            description: "atomic access count fixture".to_string(),
            allowed_agents: None,
            strategy: "round_robin".to_string(),
        }))
        .await
        .expect("vault_set_api_key_pool should succeed");
    let names = [
        "ACCESS_COUNT_ROLLBACK_API_KEY_1",
        "ACCESS_COUNT_ROLLBACK_API_KEY_2",
        "ACCESS_COUNT_ROLLBACK_API_KEY_3",
    ];
    for name in names {
        reset_access_count(&server, name);
    }
            ...
            store
                ...
                .execute_batch(
                    "CREATE TEMP TRIGGER fail_second_access_count_touch
                     ...
                     WHEN OLD.name = 'ACCESS_COUNT_ROLLBACK_API_KEY_2'
                     BEGIN
                       SELECT RAISE(ABORT, 'fixture atomic touch failure');
                     END;",
                )
                .map_err(|e| e.to_string())
        })
    ...
    let err = match crate::vault_ops::load_unlocked_api_key_secret_pools(&server) {
        Ok(_) => panic!("touch failure must fail the loader"),
        Err(err) => err,
    };
    assert!(err.contains("fixture atomic touch failure"), "{err}");
    for name in names {
        assert_eq!(
            access_count(&server, name),
            0,
            "one failed touch must roll back the entire access-count batch"
        );
    }
}
