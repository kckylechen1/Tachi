...
pub(in crate::memory_search_ops::save_memory) fn validate_save_text(
    ...
    if !params.force && memcore::is_noise_text(safe_text) {
    ...
    let gate_mode = tachi_capture_gate::GateMode::from_env();
    let gate_decision = tachi_capture_gate::evaluate(
        &tachi_capture_gate::GateInput::new(
            safe_text,
            &params.path,
