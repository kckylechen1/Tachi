...
// or with each other. `utils::global_test_lock` is the shared cfg-test mutex
...
// exclusive. The mutex guard is held for the test's lifetime and each test
// resolves a unique fixture path from the current timestamp, so concurrent
// tests cannot trample each other's run directory even though
// `TACHI_RUN_ROOT` is not restored on drop.
...
fn temp_runs_root() -> RunsRootGuard {
    ...
    //   1. The shared `utils::global_test_lock` mutex is held for the entire
