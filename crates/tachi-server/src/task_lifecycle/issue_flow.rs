...
pub(crate) fn build_issue_automation_plan(
    ...
    let recommended_next_action = if dispatch_allowed {
        "Run tachi_task(action='cycle_status', flow_id=...) to inspect lifecycle state, then use harness-native workers by default; Tachi dispatch requires an explicit durable/remote exception."
    } else {
