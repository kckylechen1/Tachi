...
Tachi 是一个单二进制、本地优先的 Agent 记忆与协调后端。它以 [MCP](https://modelcontextprotocol.io/) 服务器形态（`tachi-server` / `tachi`）运行，为 Agent 提供：
...
graph TD
    ...
    subgraph Core["核心 (Rust MemCore (memcore))"]
        API["存储 API"]
        SEARCH["五通道混合检索"]
        GRAPH["记忆图谱"]
        VAULT["Vault 元数据"]
...
| `crates/memcore-node` | Node.js 原生绑定（`@chaoxlabs/tachi-node`）。 |
