...
|---|---|---|
| 1 | Sidecar shadow process + snapshot export path; zero changes to tachi-server API or callers | **Done.** `export_snapshot.py` + `sidecar.py`; `git diff --stat origin/main` has zero `crates/` lines (see verification section of the PR). |
