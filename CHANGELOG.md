...
Distribution: the tag pipeline builds the Mac arm64 CLI tarball and promotes the public Homebrew formula (the formula installs that CLI archive; bottles are a maintainer-only rebuild path). Windows assets remain pending (#1963), and npm packages (`@chaoxlabs/tachi-node`, `@tachi/cli`) publish through their own manual workflow, so a 2.0.0 tag does not by itself mean 2.0.0 is on npm; treat the git tag and the Homebrew tap as the release of record.

### Breaking / migration

- **Removed model-facing MCP routes** (router-level, `tool not found` after upgrade), per group: the memory save/search aliases, direct Kanban routes, and duplicate wiki/brief routes retired in #1799 have their capabilities on the canonical facades (`tachi_memory(action='save'|'search')`, `tachi_task` board actions). The `tachi_orchestrator` and skill recommendation/evolution routes (`recommend_*`, `skill_evolve`, `distill_trajectory`, `prepare_capability_bundle`, standalone `run_skill`) retired in #1800/#1690 are removed without a model-facing replacement: that "second model brain" family was retired end-to-end. Memory administration (#1760) removed eight `tachi_memory` actions (`progress`, `readiness`, `delete`, `gc`, `doctor_scan`, `ingest`, `ingest_source`, `pattern_feedback`) from every model profile, so no profile can resurrect them; store maintenance stays with the operator CLI (`tachi doctor`, `tachi repair`, `tachi gc`, `tachi delete`). The Kanban Ollama classifier was dropped as an internal detail (#1737). The ordinary Lead surface is six facades (`tachi_memory`, `tachi_task`, `tachi_agent_eval`, `tachi_staff`, `tachi_gh`, `tachi_a2a`); Worker keeps five (no `tachi_agent_eval`) (#1964).
- **DB schema 28 -> 39** (sentinels v29-v39: memory outbox and destination apply, A2A mailbox with body retention, harness session attachments/spine/receipts, delivery spine, verified agent admissions, CurrentTruth, mirror-eval identity). Upgrade path: run `tachi migrate` first (plan-only by default, zero writes; `--apply` performs the sweep with per-run typed authorization, skipping libraries held by a live daemon), and back up stores before applying. Binaries refuse to open a database stamped newer than they support, so once a store is on schema 39 an older binary will not open it: restore from backup instead of rolling the binary back (#1223, #1762).
...
- **Store file rename**: every standalone-Tachi SQLite store file is renamed `memory.db` -> `tachi-memory.db` (`~/.tachi/global/`, `~/.tachi/projects/<name>/`, repo-local `.tachi/`), per the owner-ratified naming rule "system in the filename, scope stays in the directory" (#1132). A one-time rename-on-open migration handles existing installs transparently: the first `open()` of a directory still carrying the legacy filename renames it and leaves a `memory.db` -> `tachi-memory.db` symlink behind for one release window, so anything still hard-coded to the old name keeps working through this release. The `memory-server` `[[bin]]` PATH-compat alias (unused since the #890/#912 brand rename; never distributed) is retired in the same change.
