...
| 任务与回执 | `tachi_task(action=...)` | `briefing` / `doc_index` / `profiles` / `profile` / `card` / `complete` / `board` / `cycle_status` 为 memory/ledger 面；外部 worker 例外见下方 `tachi_staff`，不是默认 subagent。PR 生命周期用 `tachi_gh`。 |
