...
| 任务与回执 | `tachi_task(action=...)` | `brief` / `profiles` / `profile` / `card` / `complete` / `board` / `status` 为 memory/ledger 面；带 `flow_id`/issue/PR 引用的 `status` 返回嵌套 cycle read model。外部 worker 例外见下方 `tachi_staff`，不是默认 subagent。PR 生命周期用 `tachi_gh`。 |
