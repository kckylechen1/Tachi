...
| `delegate` | Curated worker surface: `tachi_tools`, `runtime_info`, `tachi_memory`, `tachi_web_search`, `tachi_browse`, `tachi_unstick`, `tachi_complete`, `tachi_skill`, plus compatibility `run_skill`. | Worker subagents spawned by `tachi_task(action='dispatch')`. No dispatch, no handoff. |
