...
| `coordinate` | `remember` + `coordinate` bundles: adds `tachi_handoff`, `tachi_workflow`, `tachi_orchestrator`, `tachi_agents`, `tachi_gh`, `tachi_staff`, and `tachi_verify`. `tachi_staff(action='start')` remains an explicit durable/remote exception, not the default worker launcher. | Advanced coordination and adapter workflows; not a replacement for host-native subagents. |
