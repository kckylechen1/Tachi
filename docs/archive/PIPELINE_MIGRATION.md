...
## Goal
Port the 3 Python pipeline workers (`mcp/workers/causal.py`, `mcp/workers/distiller.py`, `mcp/workers/consolidator.py`) into the Rust `tachi-server` binary so the Python server is no longer needed for pipeline functionality.
