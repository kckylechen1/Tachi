...
`tachi_task_brief` already accepts a natural-language task and returns memory/wiki
...
- Pattern/bonding projections maintain legacy `seen / hit / miss / confidence / last_seen` counters. The explicit `tachi_memory action="pattern_feedback"` route may update them; internal pattern-evidence receipts do not.
...
1. **P0 current-truth reducer is design-only**: the assertion schema, predicate-authority policy, live GitHub/deployment reconciliation, supersede/reopen/correction reduction, evidence-head staleness, and derived current-action queue described in §1.2 do not exist yet. Current timeline projections are not a substitute.
