...
|---|---|---|
| `memcore` (feature `admin` **off**) / crate `portable-kernel` | `kernel` | Canonical memory rows, graph edges, store/open/read/write contracts, access history, and scorer types. Build with `default-features = false` or depend on `portable-kernel`. Admin (vault/hub/foundry/pack) stays out of the portable graph — see [`portable-kernel-split.md`](./portable-kernel-split.md). |
