...
|---|---|---|
| `memcore` (feature `admin` **off**) / crate `memcore` (default-features = false) | `kernel` | Canonical memory rows, graph edges, store/open/read/write contracts, access history, and scorer types. Build with `default-features = false` or depend on `memcore` (default-features = false). Admin (vault/hub/foundry/pack) stays out of the portable graph — see [`memcore-split.md`](./memcore-split.md). |
