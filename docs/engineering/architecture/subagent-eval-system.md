...
`tachi_tune(action="route_simulate")` and profile-card policy consume this
matrix: high human override rates, repeated retries, failures, slow average
latency, and high average cost reduce profile confidence, while low-cost
high-quality rows can add a small efficiency bonus.
