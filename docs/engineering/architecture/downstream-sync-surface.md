...
> **Implements / anchors:** #770, #771, #790–#794, #833, #847.  
...
|---|---|---|
| **`memory-core`** | Canonical rows, edges, store, scorer types, search primitives | **Primary sync unit.** Prefer package or subtree merge of this crate alone. |
...
2. **Land portable packaging design** (Phase 1.1): feature/`memory-server-lite` or workspace package list.  
3. **HyperTachi catch-up experiment:** merge **only** `memory-core` from a recent Tachi tag into Hyperion-HyperTachi; measure conflict volume; document.  
