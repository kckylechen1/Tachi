...
**Execution-owner invariant.** These topologies describe roles and evidence gates, not a command to use Tachi as the worker launcher. The host harness's native subagent owns ordinary local execution and session lifecycle. Tachi supplies memory, policy/card advice, claims, ledger, receipts, and eval—including mirrored native-worker outcomes. The one launch-capable Tachi surface is `tachi_staff(action='start')` (the retired `tachi_task(action='dispatch')`, `tachi_shell(async_dispatch=true)`, and `tachi_arena(spawn, launch=true)` were deleted in #1319-C2/B7/D2). It is opt-in only for an explicit owner request, durable work that must outlive the current harness session, cross-device/remote pickup, or absence of a usable native subagent, expressed as the closed typed `staffing_reason` before any run artifact is created. Card/profile inspection is advisory and never converts into staffing authority by itself.
...
1. **card/profile inspection** — inspect available worker cards via `tachi_task(action="profiles"|"profile"|"card")`. The host may apply that advice to a native subagent; the result is not permission or an instruction to call Tachi dispatch.
...
- `tachi_task(action="profiles")` — inspect available worker cards/profiles.
