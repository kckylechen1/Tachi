...
**Review-recording duty (owner ruling 2026-08-03).** Every non-trivial delivery records in the PR body what the required independent review found and how each numbered finding was accepted, rejected, or downgraded. A seat, model, and bare verdict are insufficient. An owner-ratified in-house review seat can satisfy the documentation duty, but the reviewer model must still differ from the implementer's per §2.4. Missing different-model review or missing finding dispositions leaves the gate incomplete.
...
This section owns the review-diversity gate; the 2026-08-03 review-recording ruling in §2.1 adds a documentation duty for every non-trivial delivery, with no risk-class carve-out. That duty never substitutes for the different-model separation this section requires.
...
The repository pays expensive acceptance ceremony once per stable candidate, not once per repair attempt. This is a scheduling rule; it never narrows the command set owed by this canon or the packet.
...
**Acceptance-command fidelity** (leader-proposed 2026-08-03, owner unopposed). A PR acceptance claim runs and quotes every `run:` step in `ci.yml`'s `rust` job. Its `uses:` steps (checkout, Rust setup, the nextest installer, and JUnit upload) are tooling, not quoted commands. The independent `gitleaks` job remains a required secret-scan gate: quote its exact check result or local gate output, never omit it. The Windows-only `physical-db-identity-windows` job, the `node` package matrix, and `build-seat-setup`'s inline policy script are named with the reason they were not reproduced locally. Any other narrowing names exactly what was dropped and why; a filtered green never silently substitutes for the canonical surface.
