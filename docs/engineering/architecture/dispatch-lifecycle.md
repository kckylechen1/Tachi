...
**Acceptance-command fidelity** (leader-proposed 2026-08-03, owner unopposed). A PR acceptance claim runs and quotes every `run:` step in `ci.yml`'s `rust` job. Its `uses:` steps (checkout, Rust setup, the nextest installer, and JUnit upload) are tooling, not quoted commands. The Windows-only `physical-db-identity-windows` job, the `node` package matrix, and `build-seat-setup`'s inline policy script are named with the reason they were not reproduced locally. Any other narrowing names exactly what was dropped and why; a filtered green never silently substitutes for the canonical surface.
