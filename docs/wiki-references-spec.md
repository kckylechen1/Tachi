...
| `crates/memory-server/src/tool_params/facade.rs` | Add `references: Vec<String>` to `TachiWikiParams` |
...
```rust
// crates/memory-server/src/tool_params/facade.rs
...
| 2 | `tool_params/facade.rs` | Add `references: Vec<String>` with `#[serde(default)]` | 5 min |
