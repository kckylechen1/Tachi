...
# tachi-lesson-forge + tachi-build-broker + tachi-llm + tachi-foundry)
# (#1278 step ①)
#
# Usage:
...
#      -p tachi-lesson-forge -p tachi-build-broker -p tachi-llm -p tachi-foundry
#      --no-fail-fast` once
...
(
  cd "${ROOT}"
  nextest_args=(nextest run -p tachi-server -p tachi-bootstrap-tests -p tachi-contract-tests -p tachi-credential-profile -p tachi-gh-safe-merge -p tachi-github-runtime -p tachi-lesson-forge -p tachi-build-broker -p tachi-llm -p tachi-foundry --no-fail-fast --profile census --target-dir "${TARGET_DIR}")
